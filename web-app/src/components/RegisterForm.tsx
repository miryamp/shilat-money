...
export const RegisterForm: React.FC<RegisterFormProps> = ({ prefilledToken }) => {
  ...
  const oauthData = useMemo(() => {
    const data = searchParams.get('oauthData');
    if (data) {
      try {
  ...
  // Update form data when OAuth data changes
  useEffect(() => {
    if (oauthData) {
      setFormData(prev => ({
        ...prev,
  ...
  const handleUserDetailsSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const requiredFields = oauthData 
  ...
  const handleHouseholdSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // If we have OAuth data, merge it with the form data
      const registrationData = oauthData 
        ? {
            ...formData,
            isOAuthUser: true,
            provider: oauthData.provider,
            ...
            <div className="space-y-4">
              {oauthData && (
                <div className="flex items-center gap-2 p-4 bg-blue-50 rounded-md text-sm text-blue-700">
                  ...
                  Completing {oauthData.provider.charAt(0).toUpperCase() + oauthData.provider.slice(1)} Registration
                </div>
              )}
              <div>
                ...
                <input
                  ...
                  onChange={e => !oauthData && setFormData(prev => ({ ...prev, email: e.target.value }))}
                  className={`mt-1 block w-full rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 ${
                    oauthData ? 'bg-gray-100 border-gray-300 cursor-not-allowed' : 'border-gray-300'
                  }`}
                  readOnly={!!oauthData}
                />
              </div>
              {!oauthData && !formData.isOAuthUser && (
                <div>
                  <label className="block text-sm font-medium text-gray-700">
                    Password
                  </label>
